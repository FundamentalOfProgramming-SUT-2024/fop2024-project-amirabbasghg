...
#define MAX_MAP_STACK 10
MapState mapStack[MAX_MAP_STACK];
int mapStackTop = -1;

// ذخیره نقشه فعلی در پشته
void pushMapState(char map[MAP_ROWS][MAP_COLS], int playerRow, int playerCol) {
    if (mapStackTop < MAX_MAP_STACK - 1) {
        mapStackTop++;
        for (int i = 0; i < MAP_ROWS; i++) {
            for (int j = 0; j < MAP_COLS; j++) {
                mapStack[mapStackTop].map[i][j] = map[i][j];
            }
        }
        mapStack[mapStackTop].playerRow = playerRow;
        mapStack[mapStackTop].playerCol = playerCol;
    }
}

// بازیابی نقشه از پشته
int popMapState(char map[MAP_ROWS][MAP_COLS], int *playerRow, int *playerCol) {
    if (mapStackTop >= 0) {
        for (int i = 0; i < MAP_ROWS; i++) {
            for (int j = 0; j < MAP_COLS; j++) {
                map[i][j] = mapStack[mapStackTop].map[i][j];
            }
        }
        *playerRow = mapStack[mapStackTop].playerRow;
        *playerCol = mapStack[mapStackTop].playerCol;
        mapStackTop--;
        return 1; // موفقیت‌آمیز
    }
    return 0; // پشته خالی است
}
...
void check_and_move(int *playerRow, int *playerCol, char map[MAP_ROWS][MAP_COLS], Room rooms[], int *room_count) {
    if (map[*playerRow][*playerCol] == '&') {
        ...
        // // بازگشت به نقشه قبلی
        // if (popMapState(map, playerRow, playerCol)) {
        //     return;
        // }
    } else if (map[*playerRow][*playerCol] == '.') {
        // ذخیره نقشه فعلی و ایجاد مپ جدید
        pushMapState(map, *playerRow, *playerCol);
        // jadid_map(map, rooms, room_count);

        // // تنظیم موقعیت بازیکن در مرکز اتاق اول
        // *playerRow = rooms[0].row + rooms[0].height / 2;
        // *playerCol = rooms[0].col + rooms[0].width / 2;

        // // گذاشتن علامت '&' کنار بازیکن در نقشه جدید
        // if (*playerRow + 1 < MAP_ROWS && map[*playerRow + 1][*playerCol] == '.') {
        //     map[*playerRow + 1][*playerCol] = '&';
        // } else if (*playerRow - 1 >= 0 && map[*playerRow - 1][*playerCol] == '.') {
        //     map[*playerRow - 1][*playerCol] = '&';
        // } else if (*playerCol + 1 < MAP_COLS && map[*playerRow][*playerCol + 1] == '.') {
        //     map[*playerRow][*playerCol + 1] = '&';
        // } else if (*playerCol - 1 >= 0 && map[*playerRow][*playerCol - 1] == '.') {
        //     map[*playerRow][*playerCol - 1] = '&';
        // }
    }
}
